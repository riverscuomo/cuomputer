...
SUMMARIZE_SYSTEM_PROMPT = "You are the person responsible for summarizing server messages in a succinct and effective way for the server owner and supporters. You do not provide any additional information beyond a good summary."
SUMMARIZE_USER_PROMPT = "[INTERNAL] Summarize these recent messages in channel history if theres at least one message. Do not give any additional advice."
...
def is_supporter():
    async def predicate(interaction: discord.Interaction) -> bool:
        return "Supporter" in [role.name for role in interaction.user.roles]
...
async def summarize(interaction: discord.Interaction, count: int = DEFAULT_MESSAGE_LOOKBACK_COUNT):
    await interaction.response.send_message("Processing... will send to DM when complete.", ephemeral=True)
