...
class RiverpediaAPI:
    def __init__(self):
        # Fetch the Riverpedia entries once on initialization
        self.entries = fetch_entries()

    def to_alpha_num(self, s: str):
        """
        Remove characters that are not alphanumeric
        """
        return re.sub("[^0-9a-zA-Z']+", "*", s).replace("*", "")

    def build_reply(self, entry: dict):
        """
        Build a reply for Riverpedia entry
        """
        link = entry["link"]
        text = entry["entry"]

        if text.startswith("# "):
            text = text[2:]  # Clean up any unnecessary heading symbols

        return f"{text}.." + link.replace(" ", "%20")

    def get_wiki_response(self, content, pick_random=False):
        """
        Splits the content into words and looks for a Riverpedia entry that has a title or tag that matches.
        """
        print("get_wiki_response")

        if pick_random:
            return self.pick_random_entry()

        elements = content.split()
        elements = [self.to_alpha_num(x).lower()
                    for x in elements if len(x) > 2]
        elements = [
            x for x in elements if x not in resource_manager.common_words]
        ...
        return self.build_reply(entry)

    def recently_used(self, entry):
        """
        Check if an entry was used recently
        """
        title = entry["title"]
        ...
        if minutes < 240:  # Check if it was used within the last 4 hours
    ...
    def pick_random_entry(self):
        """
        Pick a random entry from Riverpedia
        """
        entry = random.choice(self.entries)
        while self.recently_used(entry):
            entry = random.choice(self.entries)
        return entry
...
# async def post_riverpedia_response(nick, message, language):
#     """
...
#     """
#     print("post_riverpedia_response")
#     try:
