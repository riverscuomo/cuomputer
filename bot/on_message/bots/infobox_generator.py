...
class InfoboxGenerator:
    ...
    def __init__(self, full_content, weezerpedia_api):
        infobox_data, infobox_type = self.extract_template_data(full_content, 'Infobox')
        
        self.infobox_data = infobox_data
        self.infobox_type = infobox_type
        self.weezerpedia_api = weezerpedia_api
        self.tracklist = self.extract_template_data(full_content, 'Track listing')[0]
    ...
    def generate_infobox(self):
        ...
        match self.infobox_type:
            case 'Album':
                # Draw thumbnail
                cover_img_url = self.weezerpedia_api.get_file_url(self.infobox_data['Cover'])
                self.draw_thumbnail(image, draw, cover_img_url)

                release_date = self.infobox_data.get('Released', 'Unknown date')
                year_re = re.search(r'\d{4}', release_date)
                if year_re:
                    release_date = release_date[:year_re.end()]
                
                # Draw essential panel
                self.draw_essential_panel(draw, self.infobox_data.get('Name', 'Untitled') or 'Untitled', release_date or 'Unknown release date')

                # Draw main panel
                album_type, color = self.album_types.get(self.infobox_data.get('Type', 'studio').lower(), ('Studio album by', (170,190,215)))
                self.draw_main_panel(draw, album_type, self.infobox_data.get('Artist', 'Unknown') or 'Unknown', color)

                margin = 45
                line_count = 0
                line_count += self.draw_feature_text(draw, 'Length:', self.infobox_data.get('Length', 'Unknown').replace('<br>', ' - '), 'Unknown', 95)
                line_count += self.draw_feature_text(draw, 'Label:', self.infobox_data.get('Label', 'Unknown').replace('<br>', ', '), 'No label', 95 + margin * line_count)
                line_count += self.draw_feature_text(draw, 'Genre:', self.infobox_data.get('Genre', 'Unknown').replace('<br>', ', '), 'Unkown', 95 + margin * line_count)
                line_count += self.draw_feature_text(draw, 'Producers:', self.infobox_data.get('Producer', 'No producer').replace('<br>', ', '), 'No producers', 95 + margin * line_count)

                # Track listing
                draw.text((430, 95 + margin * line_count + 30), 'Track listing', fill=color, font=self.fonts['bold'])
                if self.tracklist:
                    self.draw_tracklist(draw, 95 + margin * line_count + 80, color)
                else:
                    draw.text((430, 95 + margin * line_count + 80), 'No tracklist available', fill=(184,185,186), font=self.fonts['reg'])
    ...
    def draw_thumbnail(self, image, draw, thumbnail_url):
        if thumbnail_url is None:
            ...
            draw.text((200, 200), 'No cover', fill='white', font=self.fonts['bold'], anchor='mm')
            return
    ...
    def draw_essential_panel(self, draw, title, subtitle):
        ...
        name, line_count = self.get_wrapped_text(title, self.fonts['title'], 400 - 22)
        draw.multiline_text((22, 410), name or 'Unknown', fill='white', font=self.fonts['title'])

        # Subtitle
        draw.text((22, 410 + (line_count * 50) + 10), subtitle, fill=(184,185,186), font=self.fonts['small'])
    ...
    def draw_main_panel(self, draw, text, bold_text, color):
        ...
        draw.text((420, 33-(32/2)), text, fill='#23272a', font=self.fonts['reg'])
        draw.text((420+initial_width, 33-(32/2)), bold_text, fill='#23272a', font=self.fonts['bold'])
    ...
    def draw_feature_text(self, draw, label, value, no_value_text, y):
        ...
        parsed_value, line_count = self.get_wrapped_text(value, self.fonts['bold'], 1200 - 430 - label_width - 20, 2)
        draw.multiline_text((430 + label_width, y), parsed_value, fill='#eaeaeb', font=self.fonts['bold'], spacing=15)
    ...
    def draw_tracklist(self, draw, y, color):
        ...
        for i in range(1, track_count + 1):
            ...
            if i == 2 * items_per_column and track_count != i:
                draw.text((column_x, item_y), f'.. and {track_count - i + 1} more tracks', fill=color, font=self.fonts['medium-bold'])
                break

            # Draw track number
            track_nb = str(i) + '.'
            draw.text((column_x, item_y), track_nb, fill=color, font=self.fonts['medium-bold'])
            nb_text_width = draw.textlength(track_nb + ' ', font=self.fonts['medium-bold'])

            # Draw track name
            track_name = self.tracklist[f'title{i}']
            draw.text((column_x + nb_text_width, item_y), track_name, fill='#ffffff', font=self.fonts['medium-bold'])
